...
$table_engineers = TableRegistry::get('engineers');
...
<div class="panel panel-<?= $panel_type ?>">
	...
	<table class="table table-bordered table-comments">
		...
		foreach ($comments as $comment) {
			$engineer = $table_engineers->get($comment->engineer_id, ['contain' => 'Users']);
			$url = ['controller'=>'comments','action'=>'view',$engineer->id,$enterprise->id];
			$read = $comment->last->flags & Defines::COMMENT_FLAG_READ_ENGINEER ? '既読' : '未読';
			echo "<tr>";
			echo "<td class='col-xs-2'>".$this->Html->link( $engineer->code , $url )."</td>";
